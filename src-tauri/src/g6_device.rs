...
impl G6DeviceManager {
            ...
            Ok(device) => {
                ...
                *self.device.lock().unwrap() = Some(device);
                ...
                match self.read_device_state() {
                    Ok(state) => {
                        info!(
                            "Successfully read initial device state: {:02x?}",
                            &state[0..16]
                        );
    ...
    fn send_read_command(&self, command: Vec<u8>) -> Result<Vec<u8>> {
        ...
        eprintln!(
            "Sending read command: {} bytes - {:02x?}",
            command.len(),
        ...
        match device.read_timeout(&mut response, 1000) {
            Ok(bytes_read) => {
                eprintln!(
                    "  ✓ Read {} bytes response: {:02x?}",
                    bytes_read,
                    &response[0..std::cmp::min(16, bytes_read)]
                );
                Ok(response)
            }
            Err(e) => Err(anyhow::anyhow!("Failed to read device response: {}", e)),
        }
    }
    ...
    pub fn read_device_state(&self) -> Result<Vec<u8>> {
        ...
        if response.len() > 1 && response[0] == 0x5a && response[1] == 0x05 {
            ...
            eprintln!(
                "⚠ Warning: Response doesn't echo command: {:02x?}",
                &response[0..16]
            );
        }
    ...
    fn parse_and_update_settings(&self, response: &[u8]) -> Result<()> {
        ...
        eprintln!(
            "Parsing device state flags: 0x{:02x} (0b{:08b})",
            flags, flags
        );
